...
(() => {
  ...
  var defaultStateChange = Connector.onStateChanged;
  ...
  Connector.onScriptEvent = (e) => {
    if (e.data.type === "info") {
      trackDetails.artist = typeof e.data.artist === "string" ? e.data.artist : null;
      trackDetails.track = typeof e.data.title === "string" ? e.data.title : null;
      trackDetails.trackArt = typeof e.data.img === "string" ? e.data.img : null;
  ...
  Connector.getDuration = () => {
    return Number(
      document.querySelector(".micro-player > .playing > a")?.getAttribute("data-length") ?? 0
    ) / 1e3 || null;
  };
