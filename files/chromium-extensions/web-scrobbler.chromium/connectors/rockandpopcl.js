"use strict";
(() => {
  // src/connectors/rockandpopcl.ts
  var filter = MetadataFilter.createFilter({ artist: joinArtists });
  ...
  Connector.scrobblingDisallowedReason = () => {
    return Connector.getArtist()?.includes("24/7") || Connector.getTrack()?.includes("24/7") || Util.extractImageUrlFromSelectors(Connector.trackArtSelector)?.includes(
      "/fotos/original/"
    ) ? "FilteredTag" : null;
  };
