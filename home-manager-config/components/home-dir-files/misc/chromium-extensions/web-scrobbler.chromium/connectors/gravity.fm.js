...
(() => {
  ...
  Connector.isPlaying = () => {
    const playPauseButtonSelector = ".player-content .control-bar .play-pause-btn > svg";
    return document.querySelector(playPauseButtonSelector)?.getAttribute("data-icon") === "pause";
  };
  Connector.isLoved = () => {
    const saveButtonSelector = ".player-content .control-bar button.save-btn";
    return document.querySelector(saveButtonSelector)?.innerText.includes("SAVED");
  };
})();
