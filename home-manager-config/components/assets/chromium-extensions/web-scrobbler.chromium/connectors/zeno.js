...
(() => {
  ...
  function setupMainPlayer() {
    ...
    Connector.isPlaying = () => !document.querySelector("#media-player")?.paused;
    Connector.artistTrackSelector = ".fixed.w-full.bottom-0 .font-medium.truncate.hyphens-auto";
    Util.debugLog(`Setup main player; isPlaying=${Connector.isPlaying()}`);
  }
  function setupEmbedPlayer() {
    Connector.playerSelector = ".dashboard";
    Connector.isPlaying = () => !document.querySelector("#audioPlayer")?.paused;
