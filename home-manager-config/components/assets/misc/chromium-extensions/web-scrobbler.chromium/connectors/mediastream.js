...
(() => {
  ...
  Connector.isTrackArtDefault = (url) => {
    return Boolean(
      url?.includes("/show/images/") || url?.includes("/player/") || url?.match(/\/artists\/(?!station\/)/)
    );
  };
  Connector.isPodcast = () => Connector.getTrack()?.match(/^Program(a)?\s\d+/) !== null;
  Connector.isPlaying = () => {
    const onAirSelector = ".np__equalizer";
    return Util.isElementVisible(onAirSelector) && !Util.hasElementClass(onAirSelector, "np__equalizer__stop") && Util.hasElementClass(
