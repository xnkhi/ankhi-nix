...
(() => {
  ...
  Connector.artistSelector = [
    "[data-testid=player-song-author]",
    // Desktop
    "#mini-player div:not(:has(+p)) > p"
    // Mini
  ];
  Connector.getArtist = () => {
    const node = document.querySelector(Connector.artistSelector[0]) || document.querySelector(Connector.artistSelector[1]);
    return node?.textContent;
  };
  Connector.trackSelector = [
    "[data-testid=player-song-info] > div:first-child p",
    // Desktop
    "#mini-player div:has(+p) > p"
    // Mini
  ];
  Connector.trackArtSelector = [
    "[data-testid=player-song-icon] img",
    // Desktop
    "#mini-player img"
    // Mini
  ];
